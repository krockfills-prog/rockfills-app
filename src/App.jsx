...
function LocationInput({ value, onChange }) {
  ...
  const handleSel = (v) => {
    if (v === "__other__") { setFree(true); onChange(""); }
    else { setFree(false); onChange(v); }
  };
  return free ? (
    <div style={{ display: "flex", gap: 4 }}>
      ...
      <button onClick={() => { setFree(false); onChange(""); }}
        style={{ padding: "4px 8px", borderRadius: 8, border: "1.5px solid #e5e7eb", background: "#f9fafb", fontSize: 11, cursor: "pointer", color: "#6b7280", flexShrink: 0 }}>▼</button>
    </div>
  ) : (
...
function OffToggle({ checked, onChange, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }} onClick={() => onChange(!checked)}>
      ...
      <span style={{ fontSize: 11, color: checked ? "#ef4444" : "#6b7280", fontWeight: checked ? 700 : 400 }}>
        {checked ? `${label}休み` : `${label}あり`}
      </span>
    </div>
  );
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, width: 300, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <input type="password" value={pw} onChange={e => { setPw(e.target.value); setErr(""); }}
          onKeyDown={e => e.key === "Enter" && !busy && tryLogin()} placeholder="パスワード" autoFocus
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div style={{ background: "#fff", borderRadius: 18, padding: 24, width: 340, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea value={bi} onChange={e => setBi(e.target.value)} rows={8}
            style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1.5px solid #bfdbfe", background: "#eff6ff", fontSize: 13, outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: "inherit", lineHeight: 1.9 }} />
        </div>
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontSize: 12, fontWeight: 800, color: "#be185d", display: "block", marginBottom: 6 }}>🔴 女子（{parse(gi).length}名）</label>
          <textarea value={gi} onChange={e => setGi(e.target.value)} rows={8}
            style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1.5px solid #fbcfe8", background: "#fdf2f8", fontSize: 13, outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: "inherit", lineHeight: 1.9 }} />
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => onSave(parse(bi), parse(gi))} disabled={saving}
            style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: saving ? "#a5b4fc" : "linear-gradient(135deg,#1e3a8a,#312e81)", color: "#fff", fontWeight: 800, cursor: saving ? "not-allowed" : "pointer" }}>
            {saving ? "⏳ 保存中..." : "✓ 保存する"}
          </button>
        </div>
      </div>
              ...
              {byYear[y].map(d => {
                ...
                return (
                  <div key={d} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 12px", borderBottom: "1px solid #f3f4f6" }}>
                    <span style={{ fontSize: 13, color: isWeekend(d) ? "#6b7280" : "#ea580c" }}>
                      {`${dt.getMonth()+1}/${dt.getDate()}(${wd})`}
                      {isWeekend(d) && <span style={{ fontSize: 10, color: "#9ca3af", marginLeft: 4 }}>土日と重複</span>}
                    </span>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div style={{ background: "#fff", borderRadius: 18, padding: 24, width: 320, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1.5px solid #e5e7eb", background: "#f9fafb", color: "#374151", fontWeight: 700, cursor: "pointer" }}>キャンセル</button>
          <button onClick={() => onCreate(year, month, preview)} disabled={!validYear || preview.length === 0}
            style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: (!validYear || preview.length === 0) ? "#d1d5db" : "linear-gradient(135deg,#1e3a8a,#312e81)", color: "#fff", fontWeight: 800, cursor: (!validYear || preview.length === 0) ? "not-allowed" : "pointer" }}>
            作成する
          </button>
        </div>
      </div>
      ...
      <div onClick={() => setOpen(o => !o)} style={{ display: "flex", alignItems: "center", padding: "10px 13px", gap: 10, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", gap: 4, marginBottom: 4, flexWrap: "wrap", alignItems: "center" }}>
            {sameSch ? (
              <><Badge bg="#fef9c3" color="#854d0e" border="#fde68a">{s.boysLocation}</Badge><span style={{ fontSize: 11, color: "#6b7280" }}>{bTime}</span></>
            ) : (
              <>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <Badge bg="#dbeafe" color="#1e40af">男</Badge>
              <span style={{ fontSize: 12, fontWeight: (!s.boysOff && s.boys) ? 700 : 400, color: s.boysOff ? "#9ca3af" : s.boys ? "#111827" : "#9ca3af" }}>{s.boysOff ? "休み" : s.boys || "未定"}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <Badge bg="#fce7f3" color="#9d174d">女</Badge>
              <span style={{ fontSize: 12, fontWeight: (!s.girlsOff && s.girls) ? 700 : 400, color: s.girlsOff ? "#9ca3af" : s.girls ? "#111827" : "#9ca3af" }}>{s.girlsOff ? "休み" : s.girls || "未定"}</span>
            </div>
          </div>
        </div>
            ...
            <div style={{ padding: "12px 13px 14px" }}>
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  ...
                  <OffToggle checked={s.boysOff} onChange={v => upd(s.date, "boysOff", v)} label="練習" />
                </div>
                {!s.boysOff && (
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  ...
                  <OffToggle checked={s.girlsOff} onChange={v => upd(s.date, "girlsOff", v)} label="練習" />
                </div>
                {!s.girlsOff && (
                  <div style={{ display: "grid", gap: 8 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <div style={{ padding: "10px 13px 14px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                <div style={{ background: s.boysOff ? "#fef2f2" : "#eff6ff", borderRadius: 10, padding: "10px 12px", border: `1.5px solid ${s.boysOff ? "#fca5a5" : "#bfdbfe"}` }}>
                  ...
                  {s.boysOff ? <div style={{ fontSize: 13, fontWeight: 700, color: "#ef4444" }}>お休み</div>
                    : <><div style={{ fontSize: 10, color: "#6b7280", marginBottom: 2 }}>{s.boysLocation}　{bTime}</div><div style={{ fontSize: 15, fontWeight: 900, color: "#1e3a8a" }}>{s.boys || "未定"}</div></>}
                </div>
                <div style={{ background: s.girlsOff ? "#fef2f2" : "#fdf2f8", borderRadius: 10, padding: "10px 12px", border: `1.5px solid ${s.girlsOff ? "#fca5a5" : "#fbcfe8"}` }}>
                  ...
                  {s.girlsOff ? <div style={{ fontSize: 13, fontWeight: 700, color: "#ef4444" }}>お休み</div>
                    : <><div style={{ fontSize: 10, color: "#6b7280", marginBottom: 2 }}>{s.girlsLocation}　{gTime}</div><div style={{ fontSize: 15, fontWeight: 900, color: "#9d174d" }}>{s.girls || "未定"}</div></>}
                </div>
              </div>
...
export default function App() {
  ...
  useEffect(() => {
    if (!currentYM) return;
    if (!gasOk) {
      const d = localData[currentYM] || {};
      setCurrentRows(d.rows || []); setCurrentNotice(d.notice || ""); return;
    }
    (async () => {
      setLoading(true); setError(null);
      try {
        const data = await gasReq({ action: "get", ym: currentYM });
        setCurrentRows(data.rows || []); setCurrentNotice(data.notice || "");
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff", fontFamily: "'Noto Sans JP','Helvetica Neue',Arial,sans-serif", paddingBottom: 60, maxWidth: 580, margin: "0 auto" }}>
      <div style={{ background: "linear-gradient(135deg,#1e3a8a,#312e81)", padding: "16px 16px 12px", color: "#fff", position: "sticky", top: 0, zIndex: 20, boxShadow: "0 4px 20px rgba(30,58,138,0.3)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          {isAdmin
            ? <Badge bg="#fef08a" color="#713f12">⚙️ 管理者</Badge>
